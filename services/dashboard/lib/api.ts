...
export const fixDependencies = async () => {
  return claudeAPI.post('/api/fix-dependencies');
};

export const getComplexityReport = async (threshold = 5) => {
  return claudeAPI.get('/api/complexity-report', {
    params: { threshold }
  });
};

export const createResearchTask = async (prompt, context = '', model = 'claude-3-opus-20240229') => {
...
export const importTasks = async (tasks) => {
  return claudeAPI.post('/api/tasks/import', { tasks });
};
